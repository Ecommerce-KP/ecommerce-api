...
//        $object->save();
    ...
    public function list($condition, $relations = [], $relationCounts = [])
    {
        ...
        $entities = $this->getEntities($condition, $entities, $data);
        // limit result
     ...
     * @return mixed
     */
    public function getEntities(array $condition, mixed $entities, \Illuminate\Support\Collection $data)
    {
        if (count($condition) && method_exists($this, 'search')) {
            foreach ($condition as $key => $value) {
                $entities = $this->search($entities, $key, $value);
        ...
        // order list
        $orderBy = $data->has('sort') && in_array($data['sort'], $this->model->sortable, true) ? $data['sort'] : $this->model->getKeyName();
        return $entities->orderBy($orderBy, $data->has('sortType') && $data['sortType'] === 1 ? 'asc' : 'desc');
    }
}
